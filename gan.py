...
class GAN(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        inputs_gen = tf.keras.layers.Input(shape=[args.z_dim])
        for i, layer_size in enumerate(args.generator_layers):
            dense = tf.keras.layers.Dense(units=layer_size, activation='relu', name='gen_dense_' + str(i))
            x = dense(x) if i > 0 else dense(inputs_gen)
        outputs_gen = tf.keras.layers.Reshape(target_shape=[MNIST.H, MNIST.W, MNIST.C])(
            tf.keras.layers.Dense(units=MNIST.H * MNIST.W * MNIST.C, activation='sigmoid', name='gen_output')(x))
        self.generator = tf.keras.Model(inputs=inputs_gen, outputs=outputs_gen, name='Generator')
        ...
        inputs_dis = tf.keras.layers.Input(shape=[MNIST.H, MNIST.W, MNIST.C])
        x = tf.keras.layers.Flatten()(inputs_dis)
        for i, layer_size in enumerate(args.discriminator_layers):
            x = tf.keras.layers.Dense(units=layer_size, activation='relu', name='dis_dense_' + str(i))(x)
        outputs_dis = tf.keras.layers.Dense(units=1, activation='sigmoid', name='dis_out')(x)
        self.discriminator = tf.keras.Model(inputs=inputs_dis, outputs=outputs_dis, name='Discriminator')
        self.discriminator.summary()

        self.tb_callback = tf.keras.callbacks.TensorBoard(args.logdir)

    # We override `compile`, because we want to use two optimizers.
    def compile(self, discriminator_optimizer: tf.optimizers.Optimizer,
                generator_optimizer: tf.optimizers.Optimizer) -> None:
        super().compile(loss=tf.losses.BinaryCrossentropy(),
...
def main(args: argparse.Namespace) -> Dict[str, float]:
    ...
    network = GAN(args)
    network.compile(discriminator_optimizer=tf.optimizers.Adam(),
                    generator_optimizer=tf.optimizers.Adam())

    logs = network.fit(train,
                       epochs=args.epochs,
                       callbacks=[tf.keras.callbacks.LambdaCallback(on_epoch_end=network.generate),
                                  network.tb_callback])
